...
// Adds areguments and return address to the stack
// Allocates the kernel stack for the new thread
// Sets the pgdir of new thread to the parent process 
// Sets eip to the function
// Sets esp to the return address which is just a place holder and never be used
...
clone(void* func, void* arg1, void* arg2, void* stack) {
    ...
    *(int*)temp = t;
    ...
    // Allocate process. --- allocates kernel stack
    if((np = allocproc()) == 0) {
	return -1;
    }
...
// Waits for the thread to finish completion
int
join(void **stack) {
    //cprintf("Inside join in kernel/proc.c\n");
